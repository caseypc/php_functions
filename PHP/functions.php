...
/*--------------------------------------------------------------------------------
| Format Phone Numbers - returns areacode, exchange, and number
|--------------------------------------------------------------------------------*/
function formatPhone($number) {
	$onlynums = preg_replace('/[^0-9]/','',$number);
	$areacode = substr($onlynums, 0,3);
	$exchange = substr($onlynums,3,3);
	$number = substr($onlynums,6,4);
	return array(
		'areacode' => $areacode,
		'exchange' => $exchange,
		'number' => $number
...
/*--------------------------------------------------------------------------------
| Format Phone Numbers - with extensions and with a little bit of work country codes
|--------------------------------------------------------------------------------*/
function sanitize_phone($phone, $international = false) {
	...
	if ( preg_match( $alt_format, $phone, $matches ) ) {
		...
		return array(
			...
			'exchange' => $matches[5],
			'number' => $matches[6],
			'extension' => (!empty($matches[8]) ? $matches[8] : '')
		);
